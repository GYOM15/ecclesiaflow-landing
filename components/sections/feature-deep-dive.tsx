...
function MembersMockup() {
  const members = [
    { name: "Marie Dupont", role: "Louange · Groupe A", status: "Actif", color: "bg-indigo-500", statusBg: "bg-emerald-50 text-emerald-600" },
    { name: "Jean-Paul Mbeki", role: "Jeunesse · Groupe C", status: "Actif", color: "bg-emerald-500", statusBg: "bg-emerald-50 text-emerald-600" },
    { name: "Claire Bonnet", role: "Accueil · Groupe B", status: "Nouveau", color: "bg-amber-500", statusBg: "bg-amber-50 text-amber-600" },
    { name: "Esther Kone", role: "Prière · Groupe A", status: "Actif", color: "bg-violet-500", statusBg: "bg-emerald-50 text-emerald-600" },
    { name: "Thomas Martin", role: "Média · Groupe D", status: "Actif", color: "bg-rose-500", statusBg: "bg-emerald-50 text-emerald-600" },
  ];
  return (
    <div className="p-5 sm:p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="inline-flex items-center gap-0.5 text-[11px] text-indigo-500 font-medium">Filtrer <ChevronRight className="h-3 w-3" /></span>
      </div>
      <div className="space-y-2">
        {members.map((m) => (
          <div key={m.name} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50/80 border border-slate-100 hover:bg-slate-50 transition-colors">
            ...
            <div className="w-12 h-1.5 bg-emerald-200 rounded-full hidden sm:block" />
          </div>
        ))}
      </div>
    </div>
...
function CommunicationMockup() {
  const campaigns = [
    { title: "Rappel culte dimanche", sent: "623", rate: "94%", color: "bg-emerald-400" },
    { title: "Invitation retraite", sent: "412", rate: "87%", color: "bg-indigo-400" },
    { title: "Newsletter mensuelle", sent: "847", rate: "78%", color: "bg-amber-400" },
  ];
  return (
    <div className="p-5 sm:p-6">
      <div className="flex items-center justify-between mb-4">
        <span className="text-sm font-semibold text-slate-900">Centre de messages</span>
        <span className="text-[10px] bg-amber-50 text-amber-600 px-2 py-0.5 rounded-full font-medium">3 campagnes</span>
      </div>
      ...
      <div className="space-y-2.5">
        {campaigns.map((msg) => (
          <div key={msg.title} className="p-3 rounded-xl bg-slate-50/80 border border-slate-100">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-slate-900">{msg.title}</span>
              <span className="text-xs text-emerald-500 font-medium">{msg.rate} ouvert</span>
            </div>
            <div className="flex items-center gap-2">
...
function EventsMockup() {
  const events = [
    ...
    { title: "Groupe de prière", date: "Mer.", time: "19:30", attendees: "45", color: "bg-emerald-500" },
    { title: "Répétition louange", date: "Sam.", time: "14:00", attendees: "18", color: "bg-amber-500" },
    { title: "Retraite jeunesse", date: "15-17", time: "Mars", attendees: "85", color: "bg-violet-500" },
  ];
  return (
    <div className="p-5 sm:p-6">
      <div className="flex items-center justify-between mb-4">
        <span className="text-sm font-semibold text-slate-900">Événements à venir</span>
        <span className="text-[10px] bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full font-medium">Ce mois</span>
      </div>
      <div className="grid grid-cols-2 gap-2 mb-4">
        <div className="bg-indigo-50 rounded-xl p-3 border border-indigo-100/50">
          <div className="text-lg font-bold text-indigo-600">768</div>
          <div className="text-[10px] text-indigo-400">Inscrits total</div>
        </div>
        <div className="bg-emerald-50 rounded-xl p-3 border border-emerald-100/50">
          <div className="text-lg font-bold text-emerald-600 flex items-center gap-1">92% <TrendingUp className="h-3 w-3" /></div>
          <div className="text-[10px] text-emerald-400">Taux de présence</div>
        </div>
      </div>
      <div className="space-y-2">
        {events.map((evt) => (
          <div key={evt.title} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50/80 border border-slate-100">
          ...
          return (
            ...
            >
              <ScrollReveal direction={feature.reversed ? "right" : "left"} className={cn(feature.reversed && "lg:order-2")}>
                ...
                <p className="text-sm font-medium text-indigo-500 mb-2">{feature.subtitle}</p>
              ...
              <ScrollReveal direction={feature.reversed ? "left" : "right"} delay={0.15} className={cn(feature.reversed && "lg:order-1")}>
                <div className="relative">
                  {/* Diffuse glow */}
                  <div className="absolute -inset-4 rounded-3xl bg-gradient-to-br from-indigo-500/[0.06] via-indigo-400/[0.03] to-transparent blur-2xl" aria-hidden="true" />
                  <div className="absolute -inset-4 rounded-3xl bg-gradient-to-tl from-amber-400/[0.04] via-transparent to-transparent blur-2xl" aria-hidden="true" />
